...
import { LayoutDashboard, FileText, LogOut, Wallet, Building2, CreditCard } from 'lucide-react';
...
export default function ExporterLayout({ children }: ExporterLayoutProps) {
    ...
    const navigation = [
        ...
        { name: 'Invoices', href: '/exporter/invoices', icon: FileText },
        { name: 'Payments', href: '/exporter/payments', icon: CreditCard },
    ];
                    ...
                    <div className="hidden lg:block lg:col-span-3">
                        <nav className="sticky top-24 space-y-2">
                            {navigation.map((item) => {
                                ...
                                // Special case for dashboard to avoid matching everything since it's just /exporter
                                const isDashboard = item.href === '/exporter';
                                const isExactMatch = pathname === item.href;

                                // If it's dashboard, only exact match. If others, startswith is fine (e.g. invoices/new)
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-3 px-4 py-3 text-left rounded-lg transition-all ${active
                            ...
                            {navigation.map((item) => {
                                ...
                                return (
                                    <button
                                        key={item.name}
                                        onClick={() => router.push(item.href)}
                                        className={`flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-lg transition-all ${active
                                            ? 'bg-gradient-to-r from-cyan-500/20 to-teal-400/20 text-cyan-400 border border-cyan-500/30'
                                            : 'text-gray-300 hover:bg-slate-800/50 hover:text-white'
                                            }`}
                                    >
